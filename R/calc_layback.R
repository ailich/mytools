...
#' @param GPS_Source Adds offset forward/aft offset between trawl block and GPS source. Must be "None" or "GNSS_Fwd"
#' @export

calc_layback<- function(payout, depth, GPS_Source= "None", zeroed, cat_fact=1){
  if (GPS_Source != "GNSS_Fwd" & GPS_Source != "None"){stop('GPS_Source must be "None" or "GNSS_Fwd"' )}
